...
export default function ExpenseReport() {
  ...
  return (
    <AppLayout>
      <PageMeta title="Laporan Pengeluaran | MyFinance" description="Analitik dan tren pengeluaran" />
      <PageBreadcrumb pageTitle="Laporan Pengeluaran" />

      <div className="flex items-center justify-end mb-4">
        <Link href="/transactions/expense" className="inline-flex items-center gap-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/[0.05]">
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="rounded-2xl border border-error-200 dark:border-error-500/20 bg-error-50 dark:bg-error-500/10 p-4">
              ...
              <p className="text-xl font-bold text-error-700 dark:text-error-400">-{fmt(summary.grandTotal)}</p>
            </div>
            ...
            <div className="rounded-2xl border border-warning-200 dark:border-warning-500/20 bg-warning-50 dark:bg-warning-500/10 p-4">
              <p className="text-xs text-warning-600 dark:text-warning-400 mb-1">Periode Tertinggi</p>
              <p className="text-base font-bold text-warning-700 dark:text-warning-400">{summary.highestPeriod.label}</p>
              <p className="text-xs text-warning-600 dark:text-warning-400">{fmt(summary.highestPeriod.total)}</p>
            </div>
          </div>
                ...
                {byCategory.map((cat, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-4 px-5 py-3">
                      ...
                      <span className="text-sm font-semibold text-error-600 dark:text-error-400 w-36 text-right">-{fmt(cat.total)}</span>
                    </div>
                  );
                })}
              </div>
