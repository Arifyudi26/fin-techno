...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const highestPeriod = monthlyTrend.reduce((h, m) => m.total > h.total ? m : h, monthlyTrend[0] ?? { month: "-", total: 0 });
    ...
    return res.status(200).json({
      ...
      summary: {
        ...
        prevTotal,
        pctChange,
        highestPeriod: { label: highestPeriod.month, total: highestPeriod.total },
      },
      monthlyTrend,
      byCategory,
      bySource,
