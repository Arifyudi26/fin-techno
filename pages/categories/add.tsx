...
export default function AddCategory() {
  ...
  const validate = () => {
    ...
    if (!form.name.trim()) e.name = tr.validationName;
    if (!form.code.trim()) e.code = tr.validationCode;
    else if (form.code.length > 5) e.code = tr.validationCodeMax;
    setErrors(e);
      ...
      <div className="max-w-md">
        <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-white/[0.03] p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <Label>{tr.fieldName} <span className="text-error-500">*</span></Label>
              <Input type="text" placeholder={tr.fieldNamePlaceholder} value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} />
              {errors.name && <p className="mt-1 text-xs text-error-500">{errors.name}</p>}
            </div>
            <div>
              <Label>{tr.fieldCode} <span className="text-error-500">*</span></Label>
              <Input
                type="text"
                placeholder={tr.fieldCodePlaceholder}
                value={form.code}
            ...
            <div>
              ...
              <Input type="text" placeholder={tr.fieldDescriptionPlaceholder} value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} />
            </div>
