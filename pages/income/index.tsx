...
export default function IncomeReport() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await axiosGlobal.get(`/reports/income?year=${year}`);
  ...
  return (
    <AppLayout>
      <PageMeta title="Laporan Pemasukan | MyFinance" description="Analitik dan tren pemasukan tahunan" />
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="rounded-2xl border border-success-200 dark:border-success-500/20 bg-success-50 dark:bg-success-500/10 p-4">
              <p className="text-xs text-success-600 dark:text-success-400 mb-1">Total Pemasukan {year}</p>
              <p className="text-xl font-bold text-success-700 dark:text-success-400">+{formatIDR(summary.grandTotal)}</p>
            </div>
            ...
            <div className="rounded-2xl border border-brand-200 dark:border-brand-500/20 bg-brand-50 dark:bg-brand-500/10 p-4">
              <p className="text-xs text-brand-600 dark:text-brand-400 mb-1">Bulan Terbaik</p>
              <p className="text-base font-bold text-brand-700 dark:text-brand-400">{summary.bestMonth.month}</p>
              <p className="text-xs text-brand-600 dark:text-brand-400">{formatIDR(summary.bestMonth.total)}</p>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-white/[0.03] p-5 mb-6">
            <h3 className="text-sm font-semibold text-gray-800 dark:text-white/90 mb-4">Tren Pemasukan Bulanan {year}</h3>
            <ReactApexChart
              type="bar"
              height={260}
              series={[{ name: "Pemasukan", data: monthly.map((m) => m.total) }]}
              options={barOptions}
            />
          </div>
                    ...
                    return (
                      <div key={i}>
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-sm font-semibold text-success-600 dark:text-success-400">+{formatIDR(s.total)}</span>
                        </div>
                        <div className="h-1.5 rounded-full bg-gray-100 dark:bg-gray-800 overflow-hidden">
                          <div className="h-full rounded-full bg-success-500" style={{ width: `${pctOfTotal}%` }} />
                        </div>
                ...
                {byCategory.map((cat, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-4 px-5 py-3">
                      ...
                      <span className="text-sm font-semibold text-success-600 dark:text-success-400 w-36 text-right">+{formatIDR(cat.total)}</span>
                    </div>
                  );
                })}
              </div>
