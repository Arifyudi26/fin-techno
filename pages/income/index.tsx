...
export default function IncomeReport() {
        ...
        return `<div class="apexcharts-custom-tooltip" style="background:#1f2937;border:1px solid #374151;border-radius:10px;padding:10px 14px;min-width:180px;font-family:Outfit,sans-serif">
          <div style="display:flex;align-items:center;justify-content:space-between;gap:16px">
            <div style="display:flex;align-items:center;gap:6px">
              ...
              <span style="color:#9ca3af;font-size:12px">Pemasukan</span>
            </div>
            <span style="color:#22c55e;font-size:12px;font-weight:600">${new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(val)}</span>
          </div>
        </div>`;
      },
    },
  ...
  return (
    <AppLayout>
      <PageMeta
        title="Laporan Pemasukan | MyFinance"
        description="Analitik dan tren pemasukan tahunan"
      />
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 rounded-2xl border border-dashed border-gray-300 dark:border-gray-700">
          <p className="text-gray-500 dark:text-gray-400">
            Belum ada data pemasukan untuk tahun {year}
          </p>
          <Link
            href="/upload"
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-white/[0.03] p-4">
              ...
              {summary.pctChange !== null && (
                <p
                  className={`text-xs mt-0.5 ${summary.pctChange >= 0 ? "text-success-500" : "text-error-500"}`}
                >
            ...
            <div className="rounded-2xl border border-brand-200 dark:border-brand-500/20 bg-brand-50 dark:bg-brand-500/10 p-4">
              ...
              <p className="text-xs text-brand-600 dark:text-brand-400">
                {formatIDR(summary.bestMonth.total)}
              </p>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-white/[0.03] p-5 mb-6">
            <h3 className="text-sm font-semibold text-gray-800 dark:text-white/90 mb-4">
              Tren Pemasukan Bulanan {year}
            </h3>
            <ReactApexChart
              type="bar"
              height={260}
              series={[
                { name: "Pemasukan", data: monthly.map((m) => m.total) },
              ]}
              options={barOptions}
            />
          </div>
                ...
                {byCategory.map((cat, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-4 px-5 py-3">
                      ...
                      <span className="text-sm font-semibold text-success-600 dark:text-success-400 w-36 text-right">
                        +{formatIDR(cat.total)}
                      </span>
                    </div>
                  );
