...
export default function SignInForm() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      // Validasi credentials dulu
      await axiosGlobal.post("/auth/login", { email, password, checkOnly: true });
      // Kirim OTP
            ...
            {step === "otp" ? (
              ...
              <div>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-5">
                  <button type="button" onClick={() => handleOAuth("google")} className="inline-flex items-center justify-center gap-3 py-3 text-sm font-normal text-gray-700 transition-colors bg-gray-100 rounded-lg px-7 hover:bg-gray-200 hover:text-gray-800 dark:bg-white/5 dark:text-white/90 dark:hover:bg-white/10">
